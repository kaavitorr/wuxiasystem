...
export function createRollLabel(config) {
  ...
  if ( config.icon ) {
    switch ( config.type ) {
      case "check":
      case "skill":
        label = `<i class="dnd5e-icon" data-src="systems/wuxia-system/icons/svg/ability-score-improvement.svg"></i>${label}`;
        break;
      case "tool":
...
async function handlePostRequest(event, target) {
  ...
  const chatData = {
    ...
    content: await foundry.applications.handlebars.renderTemplate(
      "systems/wuxia-system/templates/chat/roll-request-card.hbs", { buttons }
    ),
