...
export async function migrateWorld({ bypassVersionCheck=false }={}) {
  ...
  game.settings.set("wuxia-system", "systemMigrationVersion", game.system.version);
...
export async function migrateSettings() {
  ...
  if ( (disableExperienceTracking !== undefined) && (levelingMode === undefined) ) {
    await game.settings.set("wuxia-system", "levelingMode", "noxp");
  }
  ...
  if ( (disableMovementAutomation !== undefined) && (movementAutomation === undefined) ) {
    await game.settings.set("wuxia-system", "movementAutomation", disableMovementAutomation ? "none" : "full");
  }
}
...
export function migrateItemData(item, itemData, migrationData, flags={}) {
  ...
  if ( (itemData.type === "spell") && !itemData.system?.sourceItem && flags.actorData?.items ) {
    ...
    if ( !grantingItemData ) {
      const cachedFor = item.getFlag("wuxia-system", "cachedFor");
      if ( cachedFor ) {
...
export async function getMigrationData() {
  const data = {};
  try {
    const icons = await fetch("systems/wuxia-system/json/icon-migration.json");
    const spellIcons = await fetch("systems/wuxia-system/json/spell-icon-migration.json");
...
function _migrateTokenImage(actorData, updateData) {
  ...
  for ( const path of ["texture.src", "prototypeToken.texture.src"] ) {
    ...
    if ( oldSystemPNG.test(v) ) {
      ...
      updateData[path] = `systems/wuxia-system/tokens/${type}/${fileName}.webp`;
    }
  }
  return updateData;
}
