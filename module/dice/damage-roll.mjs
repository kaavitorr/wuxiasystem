...
export default class DamageRoll extends BasicRoll {
  ...
  static async build(config={}, dialog={}, message={}) {
    config.critical ??= {};
    config.critical.multiplyNumeric ??= game.settings.get("wuxia-system", "criticalDamageModifiers");
    config.critical.powerfulCritical ??= game.settings.get("wuxia-system", "criticalDamageMaxDice");
