...
export default class RollConfigurationDialog extends Dialog5e {
  ...
  static PARTS = {
    formulas: {
      template: "systems/wuxia-system/templates/dice/roll-formulas.hbs"
    },
    configuration: {
      template: "systems/wuxia-system/templates/dice/roll-configuration.hbs"
    },
    buttons: {
      template: "systems/wuxia-system/templates/dice/roll-buttons.hbs"
    }
  };
  ...
  _identifyDiceTerms() {
    ...
    const identifyTerm = term => {
      ...
      for ( let i = 0; i < term.number; i++ ) dice.push({
        icon: `systems/wuxia-system/icons/svg/dice/${term.denomination}.svg`,
