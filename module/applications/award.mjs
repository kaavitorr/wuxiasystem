...
export default class Award extends Application5e {
  ...
  static PARTS = {
    award: {
      template: "systems/wuxia-system/templates/apps/award.hbs"
    }
  };
  ...
  async _prepareContext(options) {
    ...
    context.hideXP = game.settings.get("wuxia-system", "levelingMode") === "noxp";
  ...
  static prepareDestinations(destinations, savedDestinations) {
    const icons = {
      container: '<dnd5e-icon class="fa-fw" src="systems/wuxia-system/icons/svg/backpack.svg"></dnd5e-icon>',
  ...
  _saveDestinations(destinations) {
    ...
    target.setFlag("wuxia-system", "awardDestinations", destinations);
  }
  ...
  static async handleAward(message) {
    ...
    try {
      ...
      // Otherwise show the UI with defaults
        ...
        const savedDestinations = game.user.getFlag("wuxia-system", "awardDestinations");
