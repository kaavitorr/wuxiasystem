...
export default class ItemSheet5e extends PrimarySheetMixin(DocumentSheet5e) {
  ...
  static PARTS = {
    header: {
      template: "systems/wuxia-system/templates/items/header.hbs"
    },
    tabs: {
    ...
    activities: {
      template: "systems/wuxia-system/templates/items/activities.hbs",
      templates: [
        "systems/wuxia-system/templates/inventory/columns/uses.hbs",
        "systems/wuxia-system/templates/shared/activities.hbs"
      ],
      scrollable: [""]
    },
    advancement: {
      template: "systems/wuxia-system/templates/items/advancement.hbs",
      scrollable: [""]
    },
    description: {
      template: "systems/wuxia-system/templates/items/description.hbs",
      scrollable: [""]
    },
    details: {
      template: "systems/wuxia-system/templates/items/details.hbs",
      scrollable: [""]
    },
    effects: {
      template: "systems/wuxia-system/templates/items/effects.hbs",
      scrollable: [""]
    }
  };
  ...
  async _prepareEffectsContext(context, options) {
    const effectMap = {};
    const riders = [];
    const riderIds = new Set(this.item.getFlag("wuxia-system", "riders.effect") ?? []);
  ...
  _getAdvancementTags(advancement) {
    ...
    if ( advancement.classRestriction === "primary" ) {
      tags.push({
        ...
        icon: "systems/wuxia-system/icons/svg/original-class.svg"
      });
    } else if ( advancement.classRestriction === "secondary" ) {
      tags.push({
        label: "DND5E.AdvancementClassRestrictionSecondary",
        icon: "systems/wuxia-system/icons/svg/multiclass.svg"
      });
    }
    return tags;
  }
  ...
  async _onDropAdvancement(event, data) {
    ...
    if ( this.item.actor?.system.metadata?.supportsAdvancement && !game.settings.get("wuxia-system", "disableAdvancements") ) {
