...
export default class GroupActorSheet extends MultiActorSheet {
  ...
  static PARTS = {
    header: {
      template: "systems/wuxia-system/templates/actors/group/header.hbs"
    },
    tabs: {
      template: "systems/wuxia-system/templates/shared/horizontal-tabs.hbs",
      templates: ["templates/generic/tab-navigation.hbs"]
    },
    members: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/wuxia-system/templates/actors/group/members.hbs",
      templates: ["systems/wuxia-system/templates/actors/group/member.hbs"],
      scrollable: [""]
    },
    inventory: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/wuxia-system/templates/actors/group/inventory.hbs",
      templates: [
    ...
    biography: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/wuxia-system/templates/actors/group/biography.hbs",
      scrollable: [""]
    }
  };
  ...
  /** @inheritDoc */
  get inventorySource() {
    const inventorySource = this.actor.getFlag("wuxia-system", "inventorySource") ?? "group";
  ...
  async _prepareHeaderContext(context, options) {
    context.showXP = game.settings.get("wuxia-system", "levelingMode") !== "noxp";
  ...
  async _prepareMembersContext(context, options) {
    ...
    for ( const { actor } of this.document.system.members ) {
      ...
      if ( !section ) continue;
      const member = { id, type, img, name, system, uuid };
      // Pontos de Qi (energia) — substitui os Dados de Vida (DV) na ficha de grupo.
  ...
  async _prepareMemberEncumbrance(actor, context) {
    ...
    const systemUnits = game.settings.get("wuxia-system", "metricWeightUnits") ? "metric" : "imperial";
    context.encumbrance = {
      pct,
  ...
  static #onAward() {
    new Award({
      award: { savedDestinations: this.actor.getFlag("wuxia-system", "awardDestinations") },
      origin: this.actor
    }).render({ force: true });
  }
  ...
  static #onToggleInventory(event, target) {
    ...
    this.actor.setFlag("wuxia-system", "inventorySource", inventory);
  }
