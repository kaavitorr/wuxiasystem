...
import PF2eTraitConfig from "../config/resistance-config.mjs";
...
export default class BaseActorSheet extends PrimarySheetMixin(
  ...
  constructor(options={}) {
    ...
    const { width, height } = game.user.getFlag("wuxia-system", `sheetPrefs.${key}`) ?? {};
    options.position ??= {};
  ...
  static LIMITED_PARTS = {
    header: {
      template: "systems/wuxia-system/templates/actors/limited-header.hbs"
    },
    biography: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/wuxia-system/templates/actors/limited-body.hbs",
      scrollable: [""]
    }
  };
  ...
  async _prepareContext(options) {
    const context = {
      ...
      sidebarCollapsed: !!game.user.getFlag("wuxia-system", this._sidebarCollapsedKeyPath),
  ...
  _prepareSpellbook(context) {
    ...
    (context.itemCategories.spells ?? []).forEach(spell => {
      ...
      const hatsuFlag = spell.getFlag("wuxia-system", "hatsu");
      ...
      // Spells from items
      if ( spell.getFlag("wuxia-system", "cachedFor") ) {
        method = "item";
  ...
  async _prepareItemSpell(item, ctx) {
    ...
    if ( linked ) sourceLabel = linked.name;
      ...
      const advancementOrigin = item.getFlag("wuxia-system", "advancementOrigin");
  ...
  _renderSpellbook(context, options) {
    for ( const { usesSlots, pips, slot, dataset } of Object.values(context.spellbook) ) {
      ...
      pips.forEach(({ classes, n, tooltip, label, filled}) => {
        ...
        const icon = '<dnd5e-icon src="systems/wuxia-system/icons/svg/spell-slot.svg"></dnd5e-icon>';
  ...
  async _onRender(context, options) {
    ...
    if ( this.tabGroups.primary ) {
      const sidebarCollapsed = !!game.user.getFlag("wuxia-system", this._sidebarCollapsedKeyPath);
  ...
  async #changeLevel(event) {
    ...
    if ( !game.settings.get("wuxia-system", "disableAdvancements") ) {
  ...
  #saveSheetSize(position) {
    ...
    game.user.setFlag("wuxia-system", `sheetPrefs.${key}`, prefs);
  }
  ...
  static #showConfiguration(event, target) {
    ...
    switch ( target.dataset.config ) {
      ...
      case "resistance":
      case "weakness":
        return this._renderChild(new PF2eTraitConfig(config));
    }
  }
  ...
  static #toggleSidebar(event, target) {
    ...
    game.user.setFlag("wuxia-system", this._sidebarCollapsedKeyPath, collapsed);
  }
  ...
  _processFormData(event, form, formData) {
    ...
    for ( const [key, value] of Object.entries(submitData.flags?.dnd5e ?? {}) ) {
      ...
      if ( this.actor.isToken && this.actor.parent.baseActor.getFlag("wuxia-system", key) ) continue;
  ...
  async _onDropActor(event, actor) {
    const canPolymorph = game.user.isGM || (this.actor.isOwner && game.settings.get("wuxia-system", "allowPolymorphing"));
    ...
    const settings = await TransformDialog.promptSettings(this.actor, actor, {
      transform: { settings: game.settings.get("wuxia-system", "transformationSettings") },
      windowId: this.window?.windowId
    });
    if ( !settings ) return;
    await game.settings.set("wuxia-system", "transformationSettings", settings.toObject());
  ...
  async _onDropCreateItems(event, items, behavior) {
    ...
    if ( multipleAdvancements && !game.settings.get("wuxia-system", "disableAdvancements") ) {
  ...
  async _onDropSingleItem(event, itemData, { container=null }={}) {
    ...
    if ( actor.system.metadata?.supportsAdvancement && !foundry.utils.isEmpty(itemData.system.advancement)
        && !game.settings.get("wuxia-system", "disableAdvancements") ) {
