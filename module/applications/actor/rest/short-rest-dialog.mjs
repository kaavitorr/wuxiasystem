...
export default class ShortRestDialog extends BaseRestDialog {
  ...
  static PARTS = {
    ...super.PARTS,
    content: {
      template: "systems/wuxia-system/templates/actors/rest/short-rest.hbs"
    }
  };
  ...
  static async #rollEnergyDie(event, target) {
    ...
    if ( ed.value <= 0 ) {
      ui.notifications.warn("Sem Dados de Qi disponíveis!");
      return;
    }
    ...
    await roll.toMessage({
      ...
      flavor: `${actor.name} recupera Pontos de Qi (${diceSpent}× ${ed.denomination} + mod. CON)`
    });
