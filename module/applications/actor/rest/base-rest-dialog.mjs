...
export default class BaseRestDialog extends Dialog5e {
  ...
  static DEFAULT_OPTIONS = {
    ...
    templates: [
      "systems/wuxia-system/templates/actors/rest/parts/hit-dice.hbs",
      "systems/wuxia-system/templates/actors/rest/parts/rest-request.hbs"
    ]
  };
  ...
  static PARTS = {
    ...super.PARTS,
    content: {
      template: "systems/wuxia-system/templates/actors/rest/base-rest.hbs"
    }
  };
  ...
  get promptNewDay() {
    const duration = CONFIG.DND5E.restTypes[this.config.type]
      ?.duration?.[game.settings.get("wuxia-system", "restVariant")] ?? 0;
  ...
  async _prepareContext(options) {
    const context = {
      ...
      variant: game.settings.get("wuxia-system", "restVariant")
    };
    ...
    if ( this.isPartyGroup ) {
      const restSettings = this.actor.getFlag("wuxia-system", "restSettings") ?? {};
      context.request = [
        {
          field: new BooleanField({
  ...
  static async #handleFormSubmission(event, form, formData) {
    ...
    if ( this.isPartyGroup ) {
      ...
      this.actor.setFlag("wuxia-system", "restSettings", data);
    }
