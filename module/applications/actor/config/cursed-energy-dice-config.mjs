...
static DEFAULT_OPTIONS = {
  ...
  static PARTS = {
    config: {
      template: "systems/wuxia-system/templates/actors/config/cursed-energy-dice-config.hbs"
    }
  };
  ...
  static async #rollDie(event, target) {
    ...
    if (energyDice.value <= 0) {
      ui.notifications.warn(`${actor.name} não tem Dados de Qi disponíveis!`);
      return;
    }
    ...
    roll.toMessage({
      ...
      flavor: `Dados de Qi (${denomination} + mod. CON) — Recuperou ${recovered} PA`
    });
    this.render();
  }
