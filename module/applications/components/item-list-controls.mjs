...
export default class ItemListControlsElement extends MaybeAdoptable {
   ...
   */
  get prefs() {
    return game.user.getFlag("wuxia-system", `sheetPrefs.${this.app.document.type}.tabs.${this.tab}`);
  }
  ...
  async _onCycleMode(event) {
    ...
    const current = Math.max(0, modes.indexOf(game.user.getFlag("wuxia-system", flag)));
    await game.user.setFlag("wuxia-system", flag, modes[(current + 1) % modes.length]);
    if ( action === "group" ) {
      this._initGrouping();
      this._applyGrouping();
    } else {
