...
export default class BaseSettingsConfig extends Application5e {
  ...
  static PARTS = {
    config: {
      template: "systems/wuxia-system/templates/settings/base-config.hbs"
    },
    footer: {
   ...
   * @param {string} name  Setting key within the wuxia-system namespace.
   * @returns {object}
   */
  createSettingField(name) {
    const setting = game.settings.settings.get(`wuxia-system.${name}`);
    if ( !setting ) throw new Error(`Setting \`wuxia-system.${name}\` not registered.`);
    ...
    const data = {
      ...
      value: game.settings.get("wuxia-system", name)
    };
  ...
  static async #onCommitChanges(event, form, formData) {
    ...
    for ( const [key, value] of Object.entries(foundry.utils.expandObject(formData.object)) ) {
      const setting = game.settings.settings.get(`wuxia-system.${key}`);
      const current = game.settings.get("wuxia-system", key, { document: true });
      const prior = current?._source?.value ?? current;
      const updated = await game.settings.set("wuxia-system", key, value, { document: true });
