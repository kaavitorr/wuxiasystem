...
export default class CalendarSettingsConfig extends BaseSettingsConfig {
  ...
  async _prepareConfigContext(context, options) {
    const data = game.settings.get("wuxia-system", "calendarConfig");
  ...
  async _preparePreferencesContext(context, options) {
    const data = game.settings.get("wuxia-system", "calendarPreferences");
    ...
    } else if ( !game.settings.get("wuxia-system", "calendarConfig")?.enabled ) {
