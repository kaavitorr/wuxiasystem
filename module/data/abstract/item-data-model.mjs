...
export default class ItemDataModel extends SystemDataModel {
   ...
   * @type {string}
   */
  static ITEM_TOOLTIP_TEMPLATE = "systems/wuxia-system/templates/items/parts/item-tooltip.hbs";
  ...
  get advancementRootItem() {
    return this.parent?.actor?.items.get(this.parent.getFlag("wuxia-system", "advancementRoot")?.split(".")?.[0]);
  }
  ...
  async getCardData({ activity, ...enrichmentOptions }={}) {
    ...
    const context = {
      ...
      config: CONFIG.DND5E,
      controlHints: game.settings.get("wuxia-system", "controlHints"),
      ...
      description: {
        ...
        concealed: game.user.isGM && game.settings.get("wuxia-system", "concealItemDescriptions") && !description.chat
      }
    };
