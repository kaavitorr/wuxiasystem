...
export default class BaseActivityData extends foundry.abstract.DataModel {
    ...
    return {
      ...
      constantCost: new SchemaField({
        ...
        pool: new StringField({
          ...
          choices: { generated: "Qi Gerado", total: "Aura Total" }
        }),
   ...
   */
  get isRider() {
    return !!this.item.getFlag("wuxia-system", "riders.activity")?.includes(this.id);
  }
   ...
   */
  get isScaledScroll() {
    return !!this.item.getFlag("wuxia-system", "spellLevel");
  }
  ...
  _processDamagePart(damage, rollConfig, rollData, index=0) {
    ...
    const lastType = this.item.getFlag("wuxia-system", `last.${this.id}.damageType.${index}`);
