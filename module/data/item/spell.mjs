...
export default class SpellData extends ItemDataModel.mixin(ActivitiesTemplate, ItemDescriptionTemplate) {
  ...
  get linkedActivity() {
    ...
    const uuid = this.parent.getFlag("wuxia-system", "cachedFor");
    ...
    // return fromUuidSync(this.parent.getFlag("wuxia-system", "cachedFor"), { relative, strict: false }) ?? null;
  }
    ...
    if ( this.parent.actor ) {
      ...
      if ( sourceItem?.type !== "class" ) {
        ...
        if ( !grantingItem ) {
          ...
          const advancementOrigin = this.parent.getFlag("wuxia-system", "advancementOrigin");
  ...
  getRollData(...options) {
    ...
    data.item.level = data.item.level + (this.parent.getFlag("wuxia-system", "scaling")
