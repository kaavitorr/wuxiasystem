...
export default class AttributesFields {
  ...
  static get creature() {
    return {
      ...
      // Wuxia Legacy: Feridas — acumuladas em críticos/quedas; reduzem PV máximo.
      wounds: new NumberField({
        required: true, nullable: false, integer: true, min: 0, initial: 0, label: "Feridas"
      }),
  ...
  static prepareEncumbrance(rollData, { validateItem }={}) {
    ...
    if ( game.settings.get("wuxia-system", "currencyWeight") && currency ) {
  ...
  static prepareExhaustionLevel() {
    ...
    const level = exhaustion?.getFlag("wuxia-system", "exhaustionLevel");
  ...
  static prepareHitPoints(hp, { advancement=[], mod=0, bonus=0 }={}) {
    ...
    // ── Wuxia Legacy: Feridas reduzem o PV máximo efetivo. ─────────────
    // Cada ponto de Ferida subtrai 1 do effectiveMax (mínimo 1).
    const wounds = this.attributes?.wounds ?? 0;
    if ( wounds > 0 ) {
      hp.effectiveMax = Math.max(1, hp.effectiveMax - wounds);
    }
  ...
  static async onUpdateHP(changed, options, userId) {
    ...
    if ( !game.settings.get("wuxia-system", "disableConcentration") && (userId === game.userId)
