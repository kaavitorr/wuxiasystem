...
export default class TraitsField {
  ...
  static get common() {
    return {
      ...
      // Mapping<tipoDeDano|"ALL" → número>. NÃO substitui dr/dv/di do dnd5e
      // (coexistem). resistance: reduz N do dano (mínimo 1, nunca zera).
      // weakness: soma N ao dano (teto = dobro do dano original).
      resistance: new MappingField(new NumberField({
        required: true, nullable: false, integer: true, min: 0, initial: 0,
        label: "WUXIA.Resistance.Value"
      }), { label: "WUXIA.Resistance.Label" }),
      weakness: new MappingField(new NumberField({
        required: true, nullable: false, integer: true, min: 0, initial: 0,
        label: "WUXIA.Weakness.Value"
      }), { label: "WUXIA.Weakness.Label" }),
  ...
  static prepareResistImmune() {
    ...
    // Wuxia Legacy: normalização do modelo PF2e. Se houver ALL em resistance ou
    // weakness, remove os tipos específicos (ALL vence). Também limpa entradas 0
    // que possam ter vindo de formulário/efeitos.
    for ( const key of ["resistance", "weakness"] ) {
      const map = this.traits[key];
      if ( !map || typeof map !== "object" ) continue;
      const hasAll = Object.keys(map).includes("ALL");
      for ( const type of Object.keys(map) ) {
        if ( !Number.isFinite(map[type]) ) map[type] = 0;
        if ( map[type] <= 0 ) delete map[type];
        else if ( hasAll && type !== "ALL" ) delete map[type];
      }
    }
  }
