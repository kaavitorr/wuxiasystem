...
export default class TokenDocument5e extends SystemFlagsMixin(TokenDocument) {
  ...
  static registerMovementActions() {
    ...
    CONFIG.Token.movement.actions.crawl.getCostFunction = token => {
      const noAutomation = game.settings.get("wuxia-system", "movementAutomation") === "none";
      const { actor } = token;
  ...
  async _preCreate(data, options, user) {
    ...
    if ( this.actor?.system.isNPC && !this.actorLink
      ...
      const autoRoll = options.dnd5e?.autoRollNPCHP ?? game.settings.get("wuxia-system", "autoRollNPCHP");
  ...
  _onDelete(options, userId) {
    ...
    const origin = this.actor?.getFlag("wuxia-system", "summon.origin");
    if ( origin ) {
