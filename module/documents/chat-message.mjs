...
export default class ChatMessage5e extends ChatMessage {
  ...
  get shouldDisplayChallenge() {
    ...
    switch ( game.settings.get("wuxia-system", "challengeVisibility") ) {
  ...
  async renderHTML(options={}) {
    ...
    if ( foundry.utils.getType(this.system?.getHTML) === "function" ) {
      ...
      if ( game.settings.get("wuxia-system", "autoCollapseItemCards") ) {
  ...
  _collapseTrays(html) {
    let collapse;
    switch ( game.settings.get("wuxia-system", "autoCollapseChatTrays") ) {
  ...
  _highlightCriticalSuccessFailure(html) {
    ...
    const displayAttackResult = game.user.isGM || (game.settings.get("wuxia-system", "attackRollVisibility") !== "none");
    ...
    for ( let [index, d20Roll] of this.rolls.entries() ) {
      ...
      const canCrit = ["attack", "death"].includes(this.getFlag("wuxia-system", "roll.type"));
      const isAttack = this.getFlag("wuxia-system", "roll.type") === "attack";
  ...
  async _enrichChatCard(html) {
    ...
    const roll = this.getFlag("wuxia-system", "roll");
  ...
  _enrichAttackTargets(html) {
    ...
    const visibility = game.settings.get("wuxia-system", "attackRollVisibility");
    const isVisible = game.user.isGM || (visibility !== "none");
    if ( !isVisible ) return;

    const targets = this.getFlag("wuxia-system", "targets");
  ...
  _enrichDamageTooltip(rolls, html) {
    ...
    const damageOnSave = this.getFlag("wuxia-system", "roll.damageOnSave");
    if ( damageOnSave ) {
      const p = document.createElement("p");
      p.classList.add("supplement");
  ...
  getAssociatedActivity() {
    const activity = fromUuidSync(this.getFlag("wuxia-system", "activity.uuid"), { strict: false });
    if ( activity ) return activity;
    return this.getAssociatedItem()?.system.activities?.get(this.getFlag("wuxia-system", "activity.id"));
  }
   ...
   */
  getAssociatedItem() {
    const item = fromUuidSync(this.getFlag("wuxia-system", "item.uuid"), { strict: false });
    if ( item ) return item;
    const actor = this.getAssociatedActor();
    if ( !actor ) return;
    const storedData = this.getFlag("wuxia-system", "item.data") ?? this.getOriginatingMessage().getFlag("wuxia-system", "item.data");
  ...
  getOriginatingMessage() {
    return game.messages.get(this.getFlag("wuxia-system", "originatingMessage")) ?? this;
  }
}
