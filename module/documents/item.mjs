...
export default class Item5e extends SystemDocumentMixin(Item) {
  ...
  /** @override */
  static DEFAULT_ICON = "systems/wuxia-system/icons/svg/documents/item.svg";
   ...
   * mundo, ligadas por flag). O JSON ganha `flags.wuxia-system.hatsuBundle` com os filhos;
  ...
  async exportToJSON(options={}) {
    ...
    const bundle = Array.from(contents.values()).map(i => {
      ...
      delete o._id;
      delete o.folder;
      return o;   // mantém flags wuxia-system.hatsu.{slot,parent} para religar na importação
    });
    ...
    delete data.sort;
    foundry.utils.setProperty(data, "flags.wuxia-system.hatsuBundle", bundle);
   ...
   */
  get scalingIncrease() {
    return this.system?.scalingIncrease ?? this.getFlag("wuxia-system", "scaling") ?? 0;
  }
  ...
  *allApplicableEffects() {
    ...
    const messageConfig = foundry.utils.mergeObject({
      ...
      data: {
        content: await foundry.applications.handlebars.renderTemplate(
          "systems/wuxia-system/templates/chat/item-card.hbs", context
        ),
        flags: {
  ...
  async deleteDialog({ sheet, ...dialogOptions }={}, operation={}) {
    ...
    if ( this.actor?.system.metadata?.supportsAdvancement && !game.settings.get("wuxia-system", "disableAdvancements") ) {
  ...
  static async createScrollFromSpell(spell, options={}, config={}) {
    ...
    config = foundry.utils.mergeObject({
      explanation: game.user.getFlag("wuxia-system", "creation.scrollExplanation") ?? "reference",
    ...
    if ( config.dialog !== false ) {
      ...
      await game.user.setFlag("wuxia-system", "creation.scrollExplanation", config.explanation);
    }
