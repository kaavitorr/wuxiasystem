...
export default class ActiveEffect5e extends DependentDocumentMixin(ActiveEffect) {
   ...
   * @type {string}
   */
  static DEFAULT_ICON = "systems/wuxia-system/icons/svg/documents/active-effect.svg";
  ...
  get isConcealed() {
    ...
    if ( (this.id === this.constructor.ID.BLOODIED) && (game.settings.get("wuxia-system", "bloodied") === "player") ) {
  ...
  /** @inheritDoc */
  get isTemporary() {
    return !this.isConcealed && (super.isTemporary || this.getFlag("wuxia-system", "isTemporary"));
  }
  ...
  _prepareExhaustionLevel() {
    ...
    let level = this.getFlag("wuxia-system", "exhaustionLevel");
  ...
  async createRiderConditions() {
    ...
    for ( const status of this.getFlag("wuxia-system", "riders.statuses") ?? [] ) {
      riders.add(status);
    }
  ...
  async createRiderEnchantments(options={}) {
    ...
    if ( chatMessageOrigin ) {
      ...
      profile = activity?.effects.find(e => e._id === message?.getFlag("wuxia-system", "use.enchantmentProfile"));
  ...
  static onRenderActiveEffectConfig(app, html, context) {
    const element = new foundry.data.fields.SetField(new foundry.data.fields.StringField(), {}).toFormGroup({
      ...
      value: app.document.getFlag("wuxia-system", "riders.statuses") ?? [],
  ...
  addDependent(...dependent) {
    ...
    return Promise.all(dependent.map(d => d.setFlag("wuxia-system", "dependentOn", this.uuid))).then(() => this);
  }
  ...
  getDependents() {
    ...
    return (this.getFlag("wuxia-system", "dependents") || []).reduce((arr, { uuid }) => {
      let doc;
  ...
  async richTooltip(enrichmentOptions={}) {
    ...
    return {
      content: await foundry.applications.handlebars.renderTemplate(
        "systems/wuxia-system/templates/effects/parts/effect-tooltip.hbs", {
          effect: this,
