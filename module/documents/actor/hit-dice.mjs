...
export default class HitDice {
  ...
  get value() {
    ...
    // ── Wuxia Legacy: sem classes, hit dice vêm do nível de cultivo ──
    if ( !this.classes.size ) {
      const c = this.actor.system?.cultivation;
      if ( c ) {
        const nivel = ((c.rank ?? 1) - 1) * 3 + (c.stage ?? 1);
        const gastos = this.actor.getFlag?.("wuxia-system", "hdGastos") ?? 0;
        this.#value = Math.max(0, nivel - gastos);
      }
    }
    return this.#value;
  }
  ...
  get max() {
    ...
    if ( !this.classes.size ) {
      ...
      if ( c ) {
        const nivel = ((c.rank ?? 1) - 1) * 3 + (c.stage ?? 1);
        this.#max = nivel;
        this.sizes.add(8);   // d8 como denominação padrão
      }
    }
    return this.#max;
  }
