...
export default class Actor5e extends SystemDocumentMixin(Actor) {
  ...
  /** @override */
  static DEFAULT_ICON = "systems/wuxia-system/icons/svg/documents/actor.svg";
   ...
   */
  get isPolymorphed() {
    return this.getFlag("wuxia-system", "isPolymorphed") || false;
  }
  ...
  get concentration() {
    ...
    for ( const effect of this.effects ) {
      ...
      const data = effect.getFlag("wuxia-system", "item");
  ...
  *allApplicableEffects() {
    for ( const effect of super.allApplicableEffects() ) {
      ...
      if ( effect.parent?.getFlag("wuxia-system", "riders.effect")?.includes(effect.id) ) continue;
      yield effect;
    }
  }
  ...
  static async fetchExisting(uuid, options={}) {
    ...
    const localActor = game.actors.find(a => {
      ...
      return (a.getFlag("wuxia-system", "isAutoImported") || a.getFlag("wuxia-system", "summonedCopy")) // Back-compat
  ...
  async getPreferredArtwork() {
    if ( !this._preferredArtwork ) {
      const showTokenPortrait = this.getFlag("wuxia-system", "showTokenPortrait") === true;
  ...
  /** @inheritDoc */
  prepareDerivedData() {
    const origin = this.getFlag("wuxia-system", "summon.origin");
  ...
  calculateDamage(damages, options={}) {
    ...
    // Wuxia Legacy: maps de Resistência/Vulnerabilidade estilo PF2e (numéricos).
    // Coexistem com dr/dv do dnd5e; aplicados DEPOIS destes no loop abaixo.
    const resMap = this.system.traits?.resistance ?? {};
    const weakMap = this.system.traits?.weakness ?? {};
    // Retorna { amount, source }. source = "type" (resistência específica do
    // tipo) ou "all" (resistência ALL cobrindo qualquer tipo). Espelha a
    // convenção do #changeHasEffect/setActive para o card de dano consumir
    // corretamente d.active.all[category] vs d.active.type[category].
    const pf2eAmount = (map, type) => {
      const specific = map?.[type];
      if ( Number.isFinite(specific) && specific > 0 ) return { amount: specific, source: "type" };
      if ( type !== "ALL" ) {
        const all = map?.ALL;
        if ( Number.isFinite(all) && all > 0 ) return { amount: all, source: "all" };
      }
    ...
    damages.forEach(d => {
      ...
      // Aplicado após dr/dv (coexistem) e apenas para dano (não cura). Ordem do
      // PF2e: VULNERABILIDADE primeiro (soma, teto = dobro do dano que chegou),
      // DEPOIS RESISTÊNCIA (subtrai, mínimo 1). O tipo específico prevalece
      // sobre ALL; ALL sozinho cobre qualquer tipo. A marcação de active usa
      // "all" vs "type" conforme a ORIGEM (ALL vs específica), espelhando
      // #changeHasEffect — assim o card de dano mostra o toggle correto.
      const isHealing = d.type in CONFIG.DND5E.healingTypes;
      if ( !isHealing && appliedDamage > 0 ) {
        const base = appliedDamage;   // dano que chegou neste ponto (pós dr/dv/dm)
        const weak = pf2eAmount(weakMap, d.type);
        if ( weak.amount > 0 ) {
          appliedDamage = Math.min(appliedDamage + weak.amount, base * 2);
          d.active[weak.source] ??= {};
          d.active[weak.source].vulnerability = true;
        }
        const res = pf2eAmount(resMap, d.type);
        if ( res.amount > 0 ) {
          appliedDamage = Math.max(1, appliedDamage - res.amount);
          d.active[res.source] ??= {};
          d.active[res.source].resistance = true;
        }
      }
  ...
  async endConcentration(target) {
    ...
    if ( foundry.utils.getType(target) === "string" ) effect = effects.find(e => e.id === target);
      ...
      effect = effects.find(e => {
        const data = e.getFlag("wuxia-system", "item") ?? {};
  ...
  async challengeConcentration({ dc=10, ability=null }={}) {
    ...
    return ChatMessage.implementation.create({
      content: await foundry.applications.handlebars.renderTemplate(
        "systems/wuxia-system/templates/chat/roll-request-card.hbs",
        {
          buttons: [{
  ...
  _isRemarkableAthlete(ability) {
    return (dnd5e.settings.rulesVersion === "legacy") && this.getFlag("wuxia-system", "remarkableAthlete")
  ...
  async #rollSkillTool(type, config={}, dialog={}, message={}) {
    ...
    const rollConfig = foundry.utils.mergeObject({
      ...
      halflingLucky: this.getFlag("wuxia-system", "halflingLucky"),
      reliableTalent: (relevant?.value >= 1) && this.getFlag("wuxia-system", "reliableTalent")
    }, config);
  ...
  async rollDeathSave(config={}, dialog={}, message={}) {
    ...
    if ( this.getFlag("wuxia-system", "diamondSoul") ) {
      parts.push("@prof");
  ...
  getInitiativeRollConfig(options={}) {
    ...
    const tiebreaker = game.settings.get("wuxia-system", "initiativeDexTiebreaker");
    if ( tiebreaker && Number.isNumeric(ability?.value) ) parts.push(String(ability.value / 100));

    // Fixed initiative score
    const scoreMode = game.settings.get("wuxia-system", "initiativeScore");
  ...
  async initiateRest(config={}) {
    ...
    if ( !game.user.isGM && !game.settings.get("wuxia-system", "allowRests") && !config.request ) {
    ...
    config = foundry.utils.mergeObject({
      ...
      duration: restConfig.duration[game.settings.get("wuxia-system", "restVariant")],
  ...
  async _rest(config, result={}) {
    ...
    if ( config.advanceBastionTurn && game.user.isGM && game.settings.get("wuxia-system", "bastionConfiguration").enabled
  ...
  async _getRestEnergyDiceRecovery(config={}, result={}) {
    ...
    await ChatMessage.create({
      speaker: ChatMessage.getSpeaker({ actor: this }),
      content: `${this.name} recuperou ${recovered} Dados de Qi no Descanso Longo.`
    });
  }
  ...
  async transformInto(source, settings=new TransformationSetting(), options={}) {
    ...
    const allowed = game.settings.get("wuxia-system", "allowPolymorphing");
    ...
    o.flags["wuxia-system"] = o.flags["wuxia-system"] || {};
    o.flags["wuxia-system"].transformOptions = {
      ...settings.toObject(),
    ...
    const previousActorIds = this.getFlag("wuxia-system", "previousActorIds") || [];
  ...
  async revertOriginalForm(options={}) {
    ...
    const transformOptions = this.getFlag("wuxia-system", "transformOptions");
    const previousActorIds = this.getFlag("wuxia-system", "previousActorIds") ?? [];
    const isRendered = this.sheet.rendered;

    // Obtain a reference to the original actor
    const original = game.actors.get(this.getFlag("wuxia-system", "originalActor"));
    ...
    if ( this.isToken ) {
      ...
      if ( !baseActor ) {
        ui.notifications.warn(game.i18n.format("DND5E.TRANSFORM.Warning.OriginalActor", {
          reference: this.getFlag("wuxia-system", "originalActor")
        }));
        return;
      }
      const prototypeTokenData = (await baseActor.getTokenDocument()).toObject();
      const actorData = this.token.getFlag("wuxia-system", "previousActorData");
      ...
      foundry.utils.mergeObject(tokenUpdate, this.token.getFlag("wuxia-system", "previousTokenData"));
    ...
    if ( canvas.ready ) {
      ...
      const tokenUpdates = tokens.map(t => {
        ...
        foundry.utils.mergeObject(update, t.document.getFlag("wuxia-system", "previousTokenData"));
        delete update.x;
        delete update.y;
        return update;
      });
  ...
  static addDirectoryContextOptions(app, entryOptions) {
    ...
    entryOptions.push({
      ...
      callback: li => {
        game.settings.set("wuxia-system", "primaryParty", { actor: game.actors.get(li.dataset.documentId ?? li.dataset.entryId) });
      },
      condition: li => {
        const actor = game.actors.get(li.dataset.documentId ?? li.dataset.entryId);
      ...
      callback: li => {
        game.settings.set("wuxia-system", "primaryParty", { actor: null });
      },
      condition: li => {
        const actor = game.actors.get(li.dataset.documentId ?? li.dataset.entryId);
...
async _preUpdate(changed, options, user) {
  ...
  updateBloodied(options) {
    const hp = this.system.attributes?.hp;
    if ( !hp?.effectiveMax || (game.settings.get("wuxia-system", "bloodied") === "none") ) return;
  ...
  updateEncumbrance(options) {
    ...
    if ( !encumbrance || (game.settings.get("wuxia-system", "encumbrance") === "none") ) return;
    const statuses = [];
    const variant = game.settings.get("wuxia-system", "encumbrance") === "variant";
