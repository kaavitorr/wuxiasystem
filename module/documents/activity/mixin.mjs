...
export default function ActivityMixin(Base) {
  class Activity extends DependentDocumentMixin(PseudoDocumentMixin(Base)) {
    ...
    static metadata = Object.freeze({
      ...
      usage: {
        actions: {},
        chatCard: "systems/wuxia-system/templates/chat/activity-card.hbs",
    ...
    _prepareUsageConfig(config) {
      ...
      const levelingFlag = this.item.getFlag("wuxia-system", "spellLevel");
      if ( levelingFlag ) {
      ...
      if ( this.requiresConcentration && !game.settings.get("wuxia-system", "disableConcentration") ) {
        ...
        if ( linkedActivity ) {
          ...
          if ( foundry.utils.getType(results) === "Object" ) {
            ...
            if ( updates.delete.includes(linkedActivity.item.id)
              && (this.item.getFlag("wuxia-system", "cachedFor") === otherLinkedActivity?.relativeUUID) ) {
    ...
    async rollDamage(config={}, dialog={}, message={}) {
      ...
      if ( canUpdate && !foundry.utils.isEmpty(lastDamageTypes)
        ...
        await this.item.setFlag("wuxia-system", `last.${this.id}.damageType`, lastDamageTypes);
      }
    ...
    getLinkedActivity(relativeUUID) {
      ...
      relativeUUID ??= this.item.getFlag("wuxia-system", "cachedFor");
