...
export default class TransformActivity extends ActivityMixin(BaseTransformActivityData) {
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "transform",
      img: "systems/wuxia-system/icons/svg/activity/transform.svg",
   ...
   */
  get canTransform() {
    return game.user.can("ACTOR_CREATE") && (game.user.isGM || game.settings.get("wuxia-system", "allowPolymorphing"));
  }
  ...
  async _finalizeUsage(config, results) {
    ...
    if ( profile ) {
      ...
      if ( uuid ) {
        if ( results.message instanceof ChatMessage ) results.message.setFlag("wuxia-system", "transform.uuid", uuid);
  ...
  static async #transformActor(event, target, message) {
    ...
    const profileId = message.getFlag("wuxia-system", "transform.profile");
    const profile = this.profiles.find(p => p._id === profileId) || this.profiles[0];
    const uuid = message.getFlag("wuxia-system", "transform.uuid") ?? await this.queryActor(profile);
