...
export default class AttackActivity extends ActivityMixin(BaseAttackActivityData) {
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "attack",
      img: "systems/wuxia-system/icons/svg/activity/attack.svg",
  ...
  _usageChatButtons(message) {
    const buttons = [{
      ...
      icon: '<i class="dnd5e-icon" data-src="systems/wuxia-system/icons/svg/trait-weapon-proficiencies.svg" inert></i>',
      dataset: {
        action: "rollAttack"
      }
    }];
  ...
  async rollAttack(config={}, dialog={}, message={}) {
    ...
    const rollConfig = foundry.utils.mergeObject({
      ammunition: this.item.getFlag("wuxia-system", `last.${this.id}.ammunition`),
      attackMode: this.item.getFlag("wuxia-system", `last.${this.id}.attackMode`),
      elvenAccuracy: this.actor?.getFlag("wuxia-system", "elvenAccuracy")
        && CONFIG.DND5E.characterFlags.elvenAccuracy.abilities.includes(this.ability),
      halflingLucky: this.actor?.getFlag("wuxia-system", "halflingLucky"),
      mastery: this.item.getFlag("wuxia-system", `last.${this.id}.mastery`),
    ...
    if ( canUpdate && !foundry.utils.isEmpty(flags) && (this.actor && this.actor.items.has(this.item.id)) ) {
      await this.item.setFlag("wuxia-system", `last.${this.id}`, flags);
    }
    ...
    if ( canUpdate && ammoUpdate?.destroy ) {
      ...
      await attackMessage?.setFlag("wuxia-system", "roll.ammunitionData", data);
  ...
  static #rollDamage(event, target, message) {
    ...
    const attackMode = lastAttack?.getFlag("wuxia-system", "roll.attackMode");

    // Fetch the ammunition used with the last attack roll
    let ammunition;
    const actor = lastAttack?.getAssociatedActor();
    if ( actor ) {
      const storedData = lastAttack.getFlag("wuxia-system", "roll.ammunitionData");
      ammunition = storedData
        ? new Item.implementation(storedData, { parent: actor })
        : actor.items.get(lastAttack.getFlag("wuxia-system", "roll.ammunition"));
    }
